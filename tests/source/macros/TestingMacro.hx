...
class TestingMacro {
	public static function build():Array<Field> {
		...
		for(field in fields) {
			if(Lambda.find(field.meta ?? [], m -> m.name == ":noTest") == null && field.name.startsWith("test_") && field.kind.match(FFun(_))) {
			...
			switch(mainField.kind) {
				case FFun(func):
					var insteaded:Expr = macro {
						...
						${
								{
									expr: EBlock([for(fn in testFields) {expr: ECall({expr: EConst(CIdent(fn)), pos: Context.currentPos()}, []), pos: Context.currentPos()}]),
	...
	static function resolveTestFunction(field:Field) {
		...
		if(field.meta != null && field.meta.length > 0) for(meta in field.meta) {
		...
		switch(field.kind) {
			case FFun(func):
				var added = macro {
					function __shagua(idk:String, opt:String = "="):String {var e = ""; for(i in 0...idk.length) e += opt; return e;}
