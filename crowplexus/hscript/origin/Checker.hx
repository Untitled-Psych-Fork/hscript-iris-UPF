package crowplexus.hscript.origin;
...
@:allow(crowplexus.hscript.origin.Checker)
class CheckerTypes {
	...
	function addXmlType(x: haxe.rtti.CType.TypeTree, todo: Array<Void->Void>) {
		switch (x) {
			...
			case TTypedecl(t):
				...
				if (t.path == "crowplexus.hscript.origin.TypeCheck")
					td.params.reverse();
				todo.push(function() {
...
class Checker {
	...
	public static function typeStr(t: TType) {
		...
		return switch (t) {
			...
			case TType(t, args):
				if (t.name == "crowplexus.hscript.origin.TypeCheck") typeStr(args[1]); else t.name + makeArgs(args);
